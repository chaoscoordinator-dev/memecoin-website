...
const categories = [
  ...
  "Space Exploration", "Pop Culture", "Food","F1"
];
...
const CommunityLeaderboard: React.FC = () => {
  ...
  useEffect(() => {
    const initialPredictions: Prediction[] = [
      ...
      { id: "4", text: "Taylor Swift releases Chaos Coin Anthem!", category: "Music", votes: 28, submittedBy: "6Zy5...3SdQ" },
      { id: "5", text: "Charles Leclerc becomes Chaos Coin Ambassador!", category: "F1", votes: 90, submittedBy: "6Zy5...3SdQ" }
    ];
        ...
        <tr key={p.id} className={`border-b border-gray-600 hover:bg-gray-800 ${p.isWinner ? 'bg-yellow-400 text-black animate-pulse' : ''}`}>
          ...
          <td className="py-2 px-4">
            {badges.length > 0 ? badges.map((badge, idx) => (
              <span key={idx} className="inline-block bg-gradient-to-r from-green-400 to-blue-500 text-white px-3 py-1 m-1 rounded-lg wiggle-badge">                {badge}
              </span>
            )) : '—'}
          </td>
          <td className="py-2 px-4">
